...
An especially nice features is the ability to refer to secrets in a Keyvault without committing them to git. You can do that like

```
MY_SECRET="@Microsoft.KeyVault(VaultName=<keyvaultname>,SecretName=<my-secret>)"
```
where keyvaultname is the name (not fqdn) of a keyvault that you have access to e.g. by doing `az login` and my-secret is the relevant name of the secret.
